...
def halaman_riwayat(anggaran: AnggaranHarian):
    ...
    if st.button("  Refresh Riwayat"):
        st.cache_data.clear()
        st.success("Cache dibersihkan! Memuat ulang data...")  # <-- Tambahan st.success saat refresh
        st.rerun()
    ...
    if df_transaksi is None:
        ...
        # Menampilkan pesan sukses bahwa data berhasil ditarik dari database
        st.success("✨ Data transaksi berhasil dimuat terbaru!", icon="✅") # <-- Tambahan st.success disini
        ...
        # 1. Input ID Transaksi yang ingin dihapus
        id_yang_dipilih = st.number_input("Masukkan ID Transaksi yang akan dihapus:", min_value=1, step=1)

        # Inisialisasi session state untuk menyimpan status konfirmasi jika belum ada
        if "konfirmasi_hapus" not in st.session_state:
            st.session_state.konfirmasi_hapus = False

        # 2. Tombol pemicu awal
        if st.button("Hapus Transaksi Terpilih", type="secondary"):
            st.session_state.konfirmasi_hapus = True

        # 3. Logika Konfirmasi (Hanya muncul jika tombol di atas ditekan)
        if st.session_state.konfirmasi_hapus:
            st.warning(f"Apakah Anda yakin ingin menghapus transaksi dengan ID **{id_yang_dipilih}**? Tindakan ini tidak dapat dibatalkan.")
            
            col_konfirmasi, col_batal = st.columns(2)
            
            with col_konfirmasi:
                if st.button("👍 Konfirmasi Hapus", type="primary"):
                    sukses = anggaran.hapus_transaksi(id_yang_dipilih)
                    
                    if sukses:
                        # 1. Tampilkan notifikasi toast di pojok kanan bawah yang estetik
                        st.toast(f"Transaksi ID {id_yang_dipilih} Berhasil Dihapus!", icon="🗑️")
                        
                        # Atau tetap pakai st.success di sini:
                        # st.success(f"Transaksi dengan ID {id_yang_dipilih} berhasil dihapus!")
                        
                        # 2. Tahan program selama 2 detik supaya user sempat melihat notifikasinya
                        time.sleep(2)
                        
                        # 3. Baru jalankan reset cache dan rerun
                        st.session_state.konfirmasi_hapus = False
                        st.cache_data.clear()
                        st.rerun()
                    else:
