...
class AnggaranHarian:
    ...
    def get_dataframe_transaksi(self, filter_tanggal: datetime.date = None) -> pd.DataFrame:
        # 1. Tambahkan kolom 'id' ke dalam query SQL SELECT
        ...
        if not df.empty:
            ...
            # 2. Pastikan kolom 'id' diubah nama kolomnya (misal jadi 'ID') 
            #    dan masukkan ke susunan kolom akhir DataFrame yang akan ditampilkan
            df = df[['id', 'tanggal', 'kategori', 'deskripsi', 'Jumlah (Rp)']]
            df = df.rename(columns={'id': 'ID', 'tanggal': 'Tanggal', 'kategori': 'Kategori', 'deskripsi': 'Deskripsi'})
    ...
    def hapus_transaksi(self, id_transaksi: int) -> bool:
        ...
        Mengembalikan True jika sukses, False jika terjadi error.
        """
        try:
            query = "DELETE FROM transaksi WHERE id = ?"
            # Menjalankan query dengan parameter tuple (id_transaksi,)
            database.execute_query(query, (id_transaksi,))
            return True
        except Exception as e:
            # Jika terjadi error SQLite, tangkap dan kembalikan False
